...
public class FourthBoss : BaseBoss, IBossDropObstacle, IDamageable, IBossFlamethrowerAttack
{
    ...
    [BoxGroup("Firethrower Attack Graphic Rotation"), SerializeField]
    private Vector3 _firethrowerAttackGraphicRotation = new Vector3(-15f, 90f, 0f);
    ...
    private BossFlamethrower _bossFlamethrower;
    ...
    {
        base.Awake();
        _bossFlamethrower = GetComponentInChildren<BossFlamethrower>(true);
        _startFollowOffset = FollowOffset;
    ...
    protected override void PlayerRespawn_OnPlayerRespawn()
    {
        ...
        FollowOffset = _startFollowOffset;
    ...
    {
        IsHiding = false;
        AllowTeleportationToTarget = true;
        FollowPlayer = true;
        _bossFlamethrower.Deactivate();
        Vector3 followPosition = CalculateFollowPosition();
        teleportDestination.x = followPosition.x;
        KillShakeTweens();
        KillMoveTweens();
        SpawnTeleportParticles(transform);
        SpawnTeleportParticles(teleportDestination);
    ...
    {
        LookAtPlayer = false;
        IgnoreVerticalFollowDirection = true;
        LookAtTargetOffset = Vector3.zero;
        KillMoveTweens();
        GraphicTransform.rotation = Quaternion.Euler(_firethrowerAttackGraphicRotation);
        _bossFlamethrower.ActivateChargeParticles(Shake);
    }
